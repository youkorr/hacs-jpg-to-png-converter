...
async def async_setup_services(hass: HomeAssistant) -> None:
    ...
    async def convert_to_png(call: ServiceCall) -> None:
        ...
        for input_path in input_paths:
            ...
            try:
                ...
                # Check format and convert if needed (for JPG or WEBP)
                if img.format not in ["JPEG", "JPG", "WEBP", "PNG"]:
                    raise Exception(f"Unsupported file format: {img.format}")

                if img.mode != 'RGB':
                    _LOGGER.debug(f"Converting image mode from {img.mode} to RGB")
                    img = img.convert('RGB')
